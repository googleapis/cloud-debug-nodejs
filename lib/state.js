...
StateResolver.prototype.resolveVariable_ = function(name, value, isEvaluated) {
  ...
  if (value.isPrimitive() || value.isRegExp()) {
    ...
    var maxBuffer = this.config_.capture.maxDataSize;
    ...
    if (!isEvaluated && maxProps && maxProps < numKeys) {
      ...
      data.status = MESSAGE_TABLE[BUFFER_FULL_MESSAGE_INDEX].status;
    }
  } else {
...
function isEvaluated(mirror) {
  return (typeof mirror.isEvaluated === 'boolean') && mirror.isEvaluated;
}
...
StateResolver.prototype.resolveMirrorSlow_ = function(mirror) {
  ...
  var isEval = isEvaluated(mirror);
  var maxBuffer = that.config_.capture.maxDataSize;

  if (!isEval && maxProps) {
    keys = keys.slice(0, maxProps);
  }
  else if (isEval && maxBuffer > 0) {
    keys = keys.slice(0, maxBuffer);
  }
...
StateResolver.prototype.getMirrorProperties_ = function(mirror) {
  ...
  if (!isEval && maxProperties) {
    return properties.slice(0, maxProperties);
  }
  else if (isEval && maxBuffer > 0) {
    return properties.slice(0, maxBuffer);
  }
...
StateResolver.prototype.resolveMirrorProperty_ = function(property) {
  ...
  return this.resolveVariable_(name, property.value(), false);
};
