...
DebugletApi.prototype.register_ = function(errorMessage, callback) {
  ...
  var gaeModuleName = process.env.GAE_MODULE_NAME || that.config_.serviceName;
  if (gaeModuleName) {
    desc += ' module:' + gaeModuleName;
    labels[DEBUGGEE_MODULE_LABEL] = gaeModuleName;
  }

  var gaeModuleVersion = process.env.GAE_MODULE_VERSION || that.config_.serviceVersion;
  if (gaeModuleVersion) {
    desc += ' version:' + gaeModuleVersion;
    if (process.env.GAE_MODULE_VERSION !== 'default') {
      labels[DEBUGGEE_MAJOR_VERSION_LABEL] = gaeModuleVersion;
    }
  }

  if (that.config_.description) {
    desc += ' description:' + that.config_.description;
  }
