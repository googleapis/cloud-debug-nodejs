...
module.exports.create = function(logger_, config_, fileStats_) {
  ...
  singleton = {
    ...
    set: function(breakpoint, cb) {
      sourceMapper.do(function(api) {
        setBreakpoint(api.hasMappingInfo, api.mappingInfo, breakpoint, cb);
      });
    },
  ...
  function setBreakpoint(hasMappingInfo, mappingInfo, breakpoint, cb) {
    if (!v8 ||
        !breakpoint ||
    ...
    if (!hasMappingInfo(baseScriptPath)) {
      ...
      var line = breakpoint.location.line,
          column = 0,
          mapInfo = mappingInfo(baseScriptPath, line, column);
