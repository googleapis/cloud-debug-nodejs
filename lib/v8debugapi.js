...
module.exports.create = function(logger_, config_, fileStats_) {
  ...
  singleton = {
    ...
    set: function(breakpoint, cb) {
      sourceMapper.do(function(api) {
        setBreakpoint(api, breakpoint, cb);
      });
    },
  ...
  function setBreakpoint(sourcemapperApi, breakpoint, cb) {
    if (!v8 ||
        !breakpoint ||
    ...
    if (!sourcemapperApi.hasMappingInfo(baseScriptPath)) {
      ...
      var line = breakpoint.location.line,
          column = 0,
          mapInfo = sourcemapperApi.mappingInfo(baseScriptPath, line, column);
