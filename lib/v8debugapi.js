...
/** @const */ var Sourcemapper = require('./sourcemapper.js');
...
module.exports.create = function(logger_, config_, fileStats_) {
  ...
  var jsFiles = _.pickBy(fileStats_, function(value, key) {
    return /.js$/.test(key);
  });
  var mapFiles = Object.keys(fileStats_).filter(function(file) {
  ...
  config = config_;
  fileStats = jsFiles;
  sourceMapper = new Sourcemapper(mapFiles, logger);
  ...
  singleton = {
    ...
    set: function(breakpoint, cb) {
      ...
      var fullScriptPath = path.normalize(path.join(config_.workingDirectory || '',
                                          baseScriptPath));

      if (!sourceMapper.hasMappingInfo(fullScriptPath)) {
        if (!polyfill.endsWith(fullScriptPath, '.js')) {
          return setErrorStatusAndCallback(cb, breakpoint,
                  StatusMessage.BREAKPOINT_SOURCE_LOCATION,
                  messages.ERROR_COMPILING_CONDITION);
                  //messages.SOURCE_MAP_READ_ERROR);
        }

        setInternal(breakpoint, baseScriptPath, null, cb);
      }
      else {
        // the breakpoint's line number is zero-based, but
        // the source mapper expects a line number that is one-based
        var line = breakpoint.location.line + 1,
            column = 0,
            mapInfo = sourceMapper.mappingInfo(fullScriptPath, line, column);
        ...
        breakpoint.location.path = mapInfo.file;
        breakpoint.location.line = mapInfo.line;
        setInternal(breakpoint, mapInfo.file, compile, cb);
      }
    },
