...
function Sourcemapper(sourcemapPaths, logger) {
  ...
  async.parallel(callList, function(err) {
    ...
    var queue = that.waitingCallQueue;
    that.waitingCallQueue = null;
    _.forEach(queue, function(callback){
...
Sourcemapper.prototype.do = function(callback) {
  var that = this;
  if (this.waitingCallQueue === null){
    return setImmediate(function() {
      callback({
        hasMappingInfo: that.hasMappingInfo.bind(that),
        mappingInfo: that.mappingInfo.bind(that)
      });
    });
  }

  that.waitingCallQueue.push(callback);
};
