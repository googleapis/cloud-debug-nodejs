  ...
  var callList = Array.prototype.slice.call(sourcemapPaths)
                 .map(function(path) {
                   return function(callback) {
                     processSourcemap(that.infoMap_, path, callback);
                   };
                 });

  async.parallel(callList, function(err) {
    if (err){
      logger.error('An error occurred while processing the sourcemap files' +
                   err);
    }
...
/**
 * Used to invoke methods on this Sourcemapper asynchronously.  In particular, 
 * the Sourcemapper constructor is asynchronous.  However, immediately 
 * after constructing a Sourcemapper object, this method can be used to 
 * act on the constructed Sourcemapper object.
 * 
 * If the Sourcemapper hasn't finished being constructed, the function 
 * specified won't be run until the Sourcemapper has finished.  Otherwise, 
 * it is run immediately.
 * 
 * @callback {!function(!object)} callback Specifies the action to run on 
 *  this Sourcemapper.  The callback is passed an object with the 
 * `hasMappingInfo` and `mappingInfo` properties that are each methods 
 * that can be used to operate on this Sourcemapper
 */
Sourcemapper.prototype.do = function(callback) {
  var that = this;
  if (that.api_){
  ...
  this.waitingCallQueue_.push(callback);
};

// ----------- The following are not directly exposed --------------------------
...
function processSourcemap(infoMap, mapPath, callback) {
  ...
  fs.readFile(mapPath, 'utf8',
    function(err, data) {
      if (err){
        callback(new Error('Could not read sourcemap file ' + mapPath +
                      ': ' + err));
        return;
      }
      ...
      try {
        ...
        var outputBase = consumer.file ? consumer.file
            ...
            outputPath = path.normalize(path.join(parentDir, outputBase));
        ...
        if (sources.length === 0) {
          callback(new Error('No sources listed in the sourcemap file ' +
                        mapPath));
          return;
        }
        ...
        callback(new Error('An error occurred while reading the '+
...
/**
 * Builds an object that encapsulates a Sourcemapper's public API
 * 
 * @param {!Map} infoMap The map that maps input source files to
 *  SourceMapConsumer objects that are used to calculate mapping information
 * @constructor
 */
function SourcemapperApi(infoMap) {
 ...
 */
SourcemapperApi.prototype.hasMappingInfo = function(inputPath) {
 ...
 */
SourcemapperApi.prototype.mappingInfo = function(inputPath, lineNumber, colNumber) {
