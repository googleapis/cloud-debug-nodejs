...
function Sourcemapper(sourcemapPaths, logger) {
  ...
  var that = this;
  _.forEach(sourcemapPaths, function(mapPath) {
    ...
    fs.readFile(mapPath, 'utf8',
      function(err, data) {
        if (err){
          logger.error('Could not read sourcemap file ' + mapPath +
                       ': ' + err);
          return;
        }
        ...
        try {
          ...
          _.forEach(sources, function(src) {
            that.infoMap_.set(path.normalize(src), {
