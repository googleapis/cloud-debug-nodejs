...
function requestWithRetry(request, options, callback) {
  function tryRequest(attempt) {
    request(options, function(err, response, body) {
      if (isServerError(err) && attempt < MAX_RETRY_ATTEMPTS) {
...
function getProjectNumber(callback) {
  if (process.env.GCLOUD_PROJECT_NUM) {
    callback(null, process.env.GCLOUD_PROJECT_NUM);
    return;
  }

  var request = require('request');
  requestWithRetry(request, {
    url: 'http://metadata/computeMetadata/v1/project/numeric-project-id',
